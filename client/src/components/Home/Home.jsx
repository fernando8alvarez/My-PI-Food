...
export default function Home() {
    ...
    function handleFilterHealthScore(e) {
        ...
        if (e.target.value !== "sortHS") {
            const notFound = allRecipesTotal.find(el => Math.ceil(el.healthScore / 20) === Number(e.target.value));
            if (!notFound) {
                filterHS.selectedIndex=0;
                return alert("There are no recipes with that healthScore!")
            }
    ...
    return (loading || currentRecipesPerPage.length < 1) ? (
        ...
        <div className={s.container}>
            ...
            <div className={s.paginado}>
                <Paginado allRecipes={allRecipes.length} recipesPerPage={recipesPerPage} paginate={paginate} page={currentPage} />
            </div>
