...
export default function Home() {
    ...
    const currentRecipesPerPage = allRecipes.slice(indexFirstRecipe, indexLastRecipe);//[0,1,2,3,4,5,6,7,8] Arreglo con las rectas por pagina
    ...
    return currentRecipesPerPage.length < 1 ? (
        <Loading />
       ): (
        <div className={s.container}>
            <h1 className={s.tittle}>Today's Recipes</h1>
            <div>
                <SearchBar/>
            </div>
            <div className={s.menu}>
                
                    <button onClick={(e) => handleClick(e)} className={s.btn}>Reset filters</button>

                    <select name="alphabetically" onClick={(e) => handleSortAlphabetically(e)}>
                        <option value="sortAP" disabled selected hidden> Sort alphabetically</option>
                        <option value="a">A - Z</option>
                        <option value="z">Z - A</option>
                    </select>

                    <select name="typeDiet" onClick={(e) => handleFilterDiets(e)} className={s.hover}>
                        <option value="TypeDie" disabled selected hidden >Types of diet</option>
                    ...
                    <select name="HealthScore" onClick={(e) => handleSortHealthScore(e)} >
                        <option value="sortAd" disabled selected hidden>Sort healthScore</option>
                        <option value="ascendent">Ascendent</option>
                        <option value="descendent">Descendent</option>
                    </select>

                    <select name="healthScore" onClick={(e) => handleFilterHealthScore(e)} className={s.hola}>
                        <option value="sortHS" disabled selected hidden>Type health score</option>
                        <option value="1" className={s.uno}>1</option>
                    ...
                    <select name="ShowRecipe" onClick={(e) => handleFilterAddedRecipe(e)}>
            ...
            <div className={s.paginado}>
                <Paginado allRecipes={allRecipes.length} recipesPerPage={recipesPerPage} paginate={paginate} />
            </div>
                    ...
                    currentRecipesPerPage?.map(el => {
                        return (
                            <div className={s.link} >
                                <Link to={"/home/" + el.id} className={s.link}>
